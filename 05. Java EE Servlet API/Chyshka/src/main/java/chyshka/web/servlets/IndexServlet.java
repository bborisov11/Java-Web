...
public class IndexServlet extends HttpServlet {
    ...
    private final static String INDEX_HTML_FILE_PATH = "D:\\SoftUni\\Java-Web\\05. Java EE Servlet API\\Chyshka\\src\\main\\resources\\views\\index.html";
    ...
    private String formatListItems() {
        List<AllProductsViewModel> allProducts = this.productService.findAllProducts()
                .stream()
                .map(product -> this.modelMapper.map(product, AllProductsViewModel.class))
                .collect(Collectors.toList());

        StringBuilder listItems = new StringBuilder();

        allProducts.forEach(product -> listItems.append(String
                .format("<li><a href=\"/products/details?name=%s\">%s</li>",
